...
# Bounds each group of media downloads (a ROM's files, its screenshots, its two
# cover sizes), so a game with hundreds of achievement badges cannot take over
# the HTTP connection pool the rest of the scan shares.
SCAN_MEDIA_WORKERS: Final[int] = max(1, safe_int(_get_env("SCAN_MEDIA_WORKERS"), 8))
